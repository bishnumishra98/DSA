...
int solve(int n, vector<int>& heights) {
    ...
    if(n >= 2) right = solve(n-2, heights) + abs(heights[n] - heights[n-2]);   // 'if(n >= 2)' to avoid heap buffer overflow
...
int solve(int n, vector<int>& heights, vector<int>& dp) {
    ...
    if(dp[n] != -1) return dp[n];   // if an answer for dp[n] exists in dp array, return the answer.

    int left = solve(n-1, heights, dp) + abs(heights[n] - heights[n-1]);
    int right = INT_MAX;
    if(n >= 2) right = solve(n-2, heights, dp) + abs(heights[n] - heights[n-2]);

    dp[n] = min(left, right);   // store the minimum energy for reaching index 'n' in dp[n]
    return dp[n];
}
...
int frogJump_tabulation(int n, vector<int> &heights) {
    ...
    for(int i = 1; i < n; i++) {   // i runs from 0 to 'n-1' as we have re-indexed problem to 0 to 'n-1' steps.
        int left = dp[i-1] + abs(heights[i] - heights[i-1]);
        int right = INT_MAX;
        if(i >= 2) right = dp[i-2] + abs(heights[i] - heights[i-2]);
        dp[i] = min(left, right);
    }
...
int frogJump_tabulation_SO(int n, vector<int> &heights) {
    int prev = 0;
    int prev2 = 0;   // prev2 is also 0, as we will need this while computing right initially, i.e., when i=2.

    for(int i = 1; i < n; i++) {
        int left = prev + abs(heights[i] - heights[i-1]);
        int right = INT_MAX;
        if(i >= 2) right = prev2 + abs(heights[i] - heights[i-2]);
