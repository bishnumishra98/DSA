...
// points to the next node in the sequence. Unlike arrays, linked lists do not require
// contiguous memory locations. Instead, each node contains data and a reference (or link) to
// the next node in the sequence. Some properties of linked list are:
// 1) It is non contiguous, unlike arrays or vectors which are contiguous
// 2) Less wastage of memory than vectors
// 3) Insertion of element or shifting elements take constant time O(1), unlike arrays or
//    vectors which takes O(n) to execute these operations.
...
// -> Next Field (or Link): Points to the next node, i.e. stores address of next node in the
//    sequence.
