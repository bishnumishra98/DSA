...
class Solution {
public:
    int solve_going(int i, int j, vector<vector<int>>& grid) {
        // Movement: up or left
    }

    int solve_returning(int i, int j, vector<vector<int>>& grid) {
        // Movement: down or right
    }
    ...
    int cherryPickup_memoization(vector<vector<int>>& grid) {
        int n = grid.size();
        int whileGoing = solve_going(n - 1, n - 1, grid);
        int whileReturning = solve_returning(0, 0, grid);
        return whileGoing + whileReturning;
    }
...
int main() {
    ...
    Solution sol;
    cout << sol.cherryPickup_memoization(grid) << endl;
