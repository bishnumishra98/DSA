...
class Solution {
public:
    void findCombination(int index, vector<string>& ans, string ds, string mapping[], string digits) {
        ...
        // Iterating over each element of string 's'
    ...
    // S.C: O(n * 4^n)
    //                    The depth of the recursion is equal to the number of digits n, leading to a space complexity of O(n)
    //                    for the recursion stack. Each combination is stored in a vector. There are 4^n combinations, and each
    //                    combination is of length n. Therefore, the space required to store the combinations is O(n * 4^n).
